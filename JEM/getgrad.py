...
dropout_rate = 0.0
buffer_size = 10000
load_path = "./wrxac1/best_attribute_ckpt.pt"
# sgld
reinit_freq = 0.05
n_steps = 20
sgld_lr = 1.0
...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return self.transform(img), t.from_numpy(np.array(self.label_list[index][1]).astype(int)), t.from_numpy(np.array(self.label_list[index][2]).astype(int))
...
def get_data():
    ...
    data_path_train = np.load("train_data.npy")
    data_path_test = np.load("test_data.npy")
    mydataset_train = MyDataset(data_path_train, transform_train)
    mydataset_test = MyDataset(data_path_test, transform_test)
    train_sample = distributed.DistributedSampler(mydataset_train)
    test_sample = distributed.DistributedSampler(mydataset_test)
    dload_train = DataLoader(mydataset_train, batch_size=batch_size, shuffle=False, num_workers=4,
                             drop_last=True, sampler=train_sample)
    dload_test = DataLoader(mydataset_test, batch_size=batch_size, shuffle=False, num_workers=4,
                            drop_last=False, sampler=test_sample)
    np.random.seed(1234)
    data_path_train_labeled = np.load("train_data.npy")
    np.random.shuffle(data_path_train_labeled)
    mydataset_train_labeled = MyDataset(data_path_train_labeled, transform_train)
    train_sample_labeled = distributed.DistributedSampler(mydataset_train_labeled)
    dload_train_labeled = DataLoader(mydataset_train_labeled, batch_size=batch_size, shuffle=False, num_workers=4,
                                     drop_last=True, sampler=train_sample_labeled)
    dload_train_labeled = cycle(dload_train_labeled)
    return dload_train, dload_train_labeled, dload_test
...
if optimizer == "adam":
    optim = t.optim.Adam([
        {"params": f.module.energy_output.parameters(), "lr": lr},
        {"params": f.module.attribute_output.parameters(), "lr": lr}, ],
        lr=lr, betas=[.9, .999], weight_decay=weight_decay
    )
else:
    optim = t.optim.SGD([
        {"params": f.module.energy_output.parameters(), "lr": lr},
        {"params": f.module.attribute_output.parameters(), "lr": lr}, ],
        lr=lr, momentum=.9, weight_decay=weight_decay
    )

# f.eval()
# with t.no_grad():
#     print("Test load weight:")
#     correct_a_train, correct_c_train, loss_train_a, loss_train_c, loss_train = eval_classification(f, dload_train)
#     print("Epoch {}: Train Loss {}, Train Attribute Loss {}, Train Category Loss {}, Train Attribute Acc {}, Train Category Acc {}".format("Test", loss_train, loss_train_a, loss_train_c, correct_a_train, correct_c_train))
#     correct_a, correct_c, loss_a, loss_c, loss = eval_classification(f, dload_test)
#     print("Epoch {}: Test Loss {}, Test Attribute Loss {}, Test Category Loss {}, Test Attribute Acc {}, Test Category Acc {}".format("Test", loss, loss_a, loss_c, correct_a, correct_c))
# f.train()
# f.module.f.apply(fix_bn)
...
cur_iter = 0
f.module.energy_output.register_backward_hook(backward_hook)
...
print(f.module.energy_output.weight.grad)

np.save("./grad/grad.npy", grad_block)
