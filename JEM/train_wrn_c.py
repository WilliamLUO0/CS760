...
class UnF(nn.Module):
    def __init__(self, depth=28, width=10, norm=None, dropout_rate=0.0):
        ...
        self.relu = nn.ReLU()
        self.featuremap = nn.Linear(self.f.last_dim, 512)
    ...
    def forward(self, x, y=None):
        penult_z = self.f(x)
        penult_z = self.featuremap(penult_z)
        penult_z = self.relu(penult_z)
...
no_freeze = ['f', 'category_output']
freeze = ['energy_output', 'attribute_output']
...
if optimizer == "adam":
    optim = t.optim.Adam([
        {"params": f.module.f.parameters(), "lr": lr},
        {"params": f.module.category_output.parameters(), "lr": lr}, ],
...
for epoch in range(n_epochs):
    ...
    for i, batch_data_p in tqdm(enumerate(dload_train)):
        ...
        x_lab, c_lab = batch_data_lab[0], batch_data_lab[1]
        x_lab, c_lab = x_lab.cuda(non_blocking=True), c_lab.cuda(non_blocking=True)
    ...
    if epoch % eval_every == 0:
        f.eval()
        with t.no_grad():
            correct_c, loss_c = eval_classification(f, dload_test)
            print("Epoch {}: Test Loss {} Test Category Acc {}".format(epoch, loss_c, correct_c))
            correct_c_train, loss_train_c = eval_classification(f, dload_train)
            print("Epoch {}: Train Loss {} Train Category Acc {}".format(epoch, loss_train_c, correct_c_train))
