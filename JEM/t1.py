...
#         self.f = wideresnet.Wide_ResNet(depth, width, norm=norm, dropout_rate=dropout_rate)
#         self.attribute_output = nn.Linear(self.f.last_dim, 2)
#         self.category_output = nn.Linear(self.f.last_dim, 2)
#
#     def forward(self, x, y=None):
#         penult_z = self.f(x)
#         return penult_z
...
def get_data():
    ...
    print(train_inds[0])
    print(train_labeled_inds[0])
    print(dset_train[0][0].equal(dset_train_labeled[0][0]))


    print("xxxxxxxxxxxxxxxxx")
    print(train_inds[5000])
    print(train_labeled_inds[5000])
    print(dset_train[5000][0].equal(dset_train_labeled[5000][0]))
    ...
    dload_test = DataLoader(dset_test, batch_size=100, shuffle=False,
                            num_workers=4, drop_last=False)
    for index, (x_p_d, y_p) in enumerate(dload_train):
        print(index, x_p_d.shape, x_p_d[0].shape)
        print(y_p)
        break;

    for index, (x_lab, y_lab) in enumerate(dload_train_labeled):
        print(index, x_lab.shape, x_lab[0].shape)
        print(y_lab)
        break;

    x_labb, y_labb = dload_train_labeled.__next__()
    print(x_labb.shape, x_labb[0].shape)
    print(y_labb)
