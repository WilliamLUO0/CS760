...
class ScenePredictor(BasePredictor):
  ...
  def _predict(self, images):
    ...
    print(logits.shape)  # [4,365]
    print(features.shape)  # [4,512,1,1]
    print(type(features))  # tensor
    print(type(self.get_value(features)))  # numpy
    print(self.attribute_weight.shape)  # (102, 512)
    print(self.attribute_weight.T.shape)  # (512, 102)
    category_scores = self.get_value(F.softmax(logits, dim=1))  # (4, 365)
    print(category_scores.shape)

    features = self.get_value(features).squeeze(axis=(2, 3))
    attribute_scores = features.dot(self.attribute_weight.T)  # (4, 102)
